...
class Hawkes(PointP):
    ...
    def cf_cossum(self,u,t,nQ,nQ2,N=2**5):
        a = 0
        b = nQ
        qt,wt = gauss_legendre(a,b,nQ+1)
        qt = qt[:,np.newaxis]
        qs = qt[:,np.newaxis]
        v = np.arange(N)/(b-a)*np.pi
        f = np.empty((u.size,nQ+1,nQ+1),complex)
        ...
        # Transpose to match Bermudan algorithm
        return f.transpose(2,1,0)/(b-a)
